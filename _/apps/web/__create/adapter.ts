...
// Temporary type definitions during migration
...
interface NeonAdapter {
	...
	useVerificationToken(data: { identifier: string; token: string }): Promise<VerificationToken | null>;
	updateUser(user: Partial<AdapterUser>): Promise<AdapterUser>;
	createSession(data: { sessionToken: string; userId: string; expires: Date }): Promise<AdapterSession>;
	...
	unlinkAccount(data: { provider: string; providerAccountId: string }): Promise<void>;
